...
from utils.utils import get_merging_fn
import os
import pickle
...
class ModelMerge(nn.Module):
    ...
    # def compute_metrics(self, dataloader, metric_classes, save_path='metrics_version_2_backup.pkl'):
    #     """
    ...
    #     - save_path: file path to save the computed metrics.
    #
    ...
    #     """
    #     # If the metrics file exists, load and return the metrics
    ...
    def compute_metrics(self, dataloader, metric_classes, save_path='metrics_version_20_models.pkl'):
        """
        Compute pairwise alignment metrics between all graph models (self inclusive).
        - dataloader: pytorch dataloader. Dataset (or list of datasets) over which to compute metrics
        - metric_classes: dictionary whose keys are metric types, and values are metric functions.
        ...
        # Save the computed metrics to the specified path
        with open(save_path, 'wb') as f:
            pickle.dump(self.metrics, f)

        return self.metrics

    def compute_transformations(self, transform_fn, reduce_ratio, **kwargs):
        """
        ...
        node between all models. The objective of this operation is to map all disparate feature spaces
        ...
        Returns: A dictionary for transform operations to be performed at every point defined by PREFIX and POSTFIX,
        on all graph models.
        """
        start_time = time()
        self.merges = {}
        ...
        def print_memory_usage(step):
            ...
            print(f"[{step}] Memory Usage: RSS = {mem_info.rss / (1024 ** 3):.2f} GB")
        ...
        for node in tqdm(nodes, desc="Computing transformations: "):
            if self.start_at is None or node >= self.start_at:
                ...
                # Scale merge to handle things not merged
                merge = merge * len(self.graphs)  # Hack to deal with things not merged

                # Store the chunked tensors in self.merges and self.unmerges
                self.merges[node] = merge.chunk(len(self.graphs), dim=1)
                self.unmerges[node] = unmerge.chunk(len(self.graphs), dim=0)

                # Print memory usage
                print_memory_usage(f"Node {node}")

                # Explicitly free memory of the temporary variables
                del merge, unmerge
        ...
        print_memory_usage("End of computation")
    ...
    def apply_transformations(self):
        ...
        if self.graphs:
            self.merged_model = self.graphs[0].model
